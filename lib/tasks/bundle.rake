...
namespace :ci do
  namespace :travis do
    ...
    task :install do
      arch = `uname -m`.strip!
      file_name = "#{ENV['BUNDLE_ARCHIVE']}-#{arch}.tgz"
      Dir.chdir "~" do
        %x{ wget -O "remote_#{file_name}" "https://#{ENV['AWS_S3_BUCKET']}.s3.amazonaws.com/#{file_name}" }
        %x{ tar -xf "remote_#{file_name}" }
        %x{ wget -O "remote_#{file_name}.sha2" "https://#{file_name}.s3.amazonaws.com/#{file_name}.sha2" }
      end
    end
  end
end
